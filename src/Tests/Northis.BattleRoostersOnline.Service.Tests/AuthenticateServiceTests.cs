...
namespace Northis.BattleRoostersOnline.Service.Tests
{
	...
	public class AuthenticateServiceTests : ServiceModuleTests
	{
		...
		{
			var result = await AuthenticateService.LogInAsync(login, password, CallbackAuth.Object);
		...
		{
			var result = await AuthenticateService.RegisterAsync(login, password, CallbackAuth.Object);
		...
		{
			var token = await AuthenticateService.RegisterAsync(login, password, CallbackAuth.Object);

			var logOutStatus = await AuthenticateService.LogOutAsync(token);
